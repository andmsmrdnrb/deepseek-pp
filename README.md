...
1.11.1 聚焦 MCP 工具连接的兼容性、失败恢复和大工具集管理，让现有服务接入更稳定，也让用户能更精细地控制工具何时出现在对话中。

| 方向 | 主要变化 |
|------|----------|
| MCP 协议兼容 | 兼容更多仍在使用的 MCP 服务版本，减少连接成功但工具不可用的情况。 |
| 工具调用恢复 | MCP 工具发现或调用遇到问题后，刷新和后续对话可恢复到可用状态，工具结果返回后可继续完成同一轮任务。 |
| 大型 MCP 服务管理 | 单个 MCP 服务可选择直接展示、按需适配或按需调用，更适合工具数量较多的服务，同时保留用户对实际工具的控制。 |
| 运行一致性 | 手动对话、Agent、侧边栏和自动化任务使用一致的工具可用性与授权规则。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

<details>
<summary>展开历史版本变更回顾（1.11.0 - 0.2.0）</summary>
